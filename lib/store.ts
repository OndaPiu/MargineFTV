...
async function request(path:string,init:RequestInit={}){const base=url();const k=key();if(!base||!k)throw new Error('Supabase non configurato. Inserisci le variabili dell’app.');const headers=new Headers(init.headers);headers.set('apikey',k);headers.set('Authorization',`Bearer ${k}`);headers.set('Content-Type','application/json');const res=await fetch(`${base}/rest/v1/${path}`,{...init,headers,cache:'no-store'});if(!res.ok)throw new Error(`Supabase ha restituito un errore (${res.status}).`);return res.status===204?null:res.json()}
