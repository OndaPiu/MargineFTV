export type Practice = {
 ...
 amount:number|null; probability:number|null; commission:number|null; closeDate:string; marginDate:string; cost:number|null; commissionRate:number|null; taxable:number|null; secondMarginPercent:number|null;
...
export const blankPractice=(id:string):Practice=>({id,name:'',agent:'',state:'IN TRATTATIVA',measure:'',kind:'S',source:'Manuale',customerCode:'',contractCode:'',contactId:'',description:'',customerType:'',partner:'',list:'',power:null,storage:null,panel:'',battery:'',quantity:1,amount:null,probability:null,commission:null,closeDate:'',marginDate:'',cost:null,commissionRate:8,taxable:null,secondMarginPercent:null,historical:null,revenues:[],row:null,review:[]});
...
export function metrics(p:Practice){if(p.historical)return p.historical;const contract=weighted(p);const cost=p.cost??(contract==null?null:Math.round(contract*.8));const margin=contract==null||cost==null?null:contract-cost;const commission=p.commission??(contract==null||p.commissionRate==null?null:Math.round(contract*p.commissionRate/100));return {contract,cost,margin,commission,secondMargin:margin==null||commission==null?null:margin-commission};}
